  ...
  function render(container, {hco3, cl, ph, uCl, weight}){
    ...
    if(weight!=null && cl!=null){
      ...
      if(deficit>0){
        ...
        row('Isotonic saline needed', f1(vol), 'L', `Deficit ÷ 154 mEq/L (Cl⁻ in 0.9% NaCl). Infuse with KCl (check Mg²⁺ first — hypomagnesemia can make hypokalemia refractory to K⁺ replacement); ~100 mL/hr above hourly fluid losses is sufficient.`);
      } else {
    ...
    if(severe){
      if(weight!=null){
        ...
        row('<span class="fa">Severe alkalosis</span> — H⁺ deficit', f1(hDef), 'mEq',
          `HCO₃⁻ ${f1(hco3)}${ph!=null?` / pH ${ph.toFixed(2)}`:''} exceeds the threshold for HCl infusion (HCO₃⁻ &gt; 50 or pH &gt; 7.55): 0.5 × ${f1(weight)} kg × (${f1(hco3)} − 24).`);
        ...
        row('<span class="fa">Severe alkalosis</span> (HCO₃⁻ &gt; 50 or pH &gt; 7.55)', 'HCl infusion may be indicated', '', 'Enter weight (kg) in the Patient section above to calculate the H⁺ deficit and required 0.1N HCl volume.');
      }
    }
